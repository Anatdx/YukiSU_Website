...
<!-- 应该是 3.4 版本，但 backslashxx 的 syscall manual hook 无法在 YukiSU 中使用-->
...
3. **Tracepoint Hook:**
   ...
   - 自 YukiSU commit [49b01aad](https://github.com/Anatdx/YukiSU/commit/49b01aad74bcca6dba5a8a2e053bb54b648eb124) 引入的 hook 方法
...
将 KernelSU（YukiSU）添加到内核源代码树的步骤的运行命令将被替换为：
...
```sh [bash]
curl -LSs "https://raw.githubusercontent.com/Anatdx/YukiSU/main/kernel/setup.sh" | bash -s susfs-{{branch}}
```
...
- `main` (susfs-main)
- `test` (susfs-test)
- 版本号 (例如: susfs-1.5.7, 你需要在 [分支](https://github.com/Anatdx/YukiSU/branches) 里找到它)
